...
| `poems/categories.json` | Theme-category definitions (`key`, zh/en label, optional `group`, bilingual desc; poem counts are computed live by the app, not stored). Each poem's `tags` array holds these keys (multi-label). Powers the **按主题** (theme) view in `poems.html`: index has a 按年级/按主题 toggle; theme view lists each category (title·author·first line) and expands to a grid of full-text **compare cards**. 18 categories: 送别/思乡/亲情·相思/山水/田园/咏物/边塞/爱国/节日/咏月/童趣/哲理/劳动悯农/文言寓言 + 季节(春夏秋冬). **山水 is a catch-all** — the tagger drops it from any poem that also has a more specific tag, so it holds ~11 pure-scenery poems, not every poem with a view. |
