...
| `poems/categories.json` | Theme-category definitions (`key`, zh/en label, optional `group`, bilingual desc; poem counts are computed live by the app, not stored). Each poem's `tags` array holds these keys (multi-label). Powers the **按主题** (theme) view in `poems.html`: index has a 按年级/按主题 toggle; theme view lists each category (title·author·first line) and expands to a grid of full-text **compare cards**. 19 categories: 送别/思乡/怀人/山水/田园/隐逸/咏物/边塞/爱国/节日/咏月/童趣/哲理/劳动悯农/文言寓言 + 季节(春夏秋冬). The three relationship tags are disjoint by precedence: a parting event → 送别; else longing for home → 思乡; else missing a specific person → 怀人. **山水 = genuine scenery only** (~8 poems); a poem also carrying a more specific tag drops 山水 (the specific tag wins). Poems matching **no** theme (e.g. 夜宿山寺 夸张, 画 题画) are left **untagged** — an empty `tags` array is allowed; they show in the 按年级 view but under no 按主题 category. |
